...
- GitHub publication uses the repository-installed `GLoops Autonomous Delivery` GitHub App. Its private key stays root-only on the host; every activation mints a one-hour write token for Hermes and a separate read-only token for the trusted projector, each restricted to the private `gloopsAI/gloops-paperclip-plugin` repository. Tokens are revoked and removed during shutdown. No Zach user credential or GitHub organization seat is mounted into either runtime.
