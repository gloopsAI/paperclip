...
- Hermes container initialization resets the writable workspace owner. The sidecar start barrier therefore waits for authenticated health, restores the setgid `10000:985` shared transaction root, and proves Paperclip uid:gid `995:985` can create and remove the adjacent restore lock while still reading the exact plugin pilot repository before Paperclip activation can pass preflight.
...
- A claim-time task execution gate applies one atomic run, retry, token, and wall-time budget across scheduler, continuation, and recovery paths. The controlled-swarm profile permits three runs and two retries per task, with fixed task ceilings of 50,000 input tokens, 16,000 output tokens, and 3,600,000 milliseconds. Each provider invocation defaults to 30,000 input tokens, 8,000 output tokens, eight turns, and 32 tool calls. Paperclip caps every effective explicit task envelope at 32 turns before reservation; Hermes' matching 32-turn setting is only the outer host safety ceiling, so it cannot silently truncate an admitted envelope and does not itself grant additional spend. A transaction-level company gate permits no more than four running heartbeats, and a fixed campaign cutoff makes all older issues ineligible across assignment, queued-run, scheduler, continuation, watchdog, and recovery paths. An exhausted task is terminally denied before adapter invocation; only an explicit user-authored reset epoch opens a new task budget.
- The global heartbeat/routine scheduler remains disabled. The single-flight bounded execution-recovery driver is commissioned as a process-loss backstop: it may only reap stale running rows, promote already-admitted scheduled retries, and drain already-admitted queued runs. Company queue pumps remain serialized for their full duration so slow drains cannot defeat cross-agent round-robin fairness; claim-time cancellation defers its follow-on drain until the owning pump and agent lock can release, preventing self-reentrant deadlock. The driver does not tick agent timers, routines, issue monitors, watchdogs, productivity reviews, or other broad reconciliation paths. The issue-created cutoff prevents historical work from becoming eligible, and the source-controlled environment switch is the kill switch.
- Campaign-bound trainer claims obtain a receipt from the root-owned campaign deadman over its read-only Unix-socket mount. The first eligible claim atomically writes a root-owned, mode-0600, immutable epoch whose deadline is fixed at that instant from the configured campaign duration; that duration is bounded at both ends by the broker (at least 1 hour, at most 30 days) so re-authorization cannot be deferred indefinitely, and the installed unit currently configures 24 hours. Later campaign claims reuse that deadline, and campaign-side service, process, host, marker, or partial-result restarts cannot renew it. Missing, malformed, mismatched, or expired receipts roll a campaign claim back before adapter invocation. A campaign-bound invocation must use strict execution budgeting, and its wall-time ceiling is capped to the remaining epoch so in-flight work cannot outlive the deadman; adapters without that enforcement are refused before provider invocation. The dedicated `paperclip-controlled-swarm.service`, isolated handshake, and controlled-swarm recovery path load `campaign-runtime.env` and remain bound to the deadman. Controlled-swarm activation starts that dedicated service, never the general control plane, while staging the campaign-free activation marker for the return handoff. Expiry fences and masks the campaign service, clears and revokes its projected GitHub installation credential before container teardown, clears only its campaign marker, and persists a root-owned product-restoration obligation until the general plane is verified healthy and its monotonic stop receipt is committed. Projector cleanup therefore completes even when general restoration fails, and broker retries cannot lose or falsely downgrade the handoff. The general Hermes execution sidecar and registered GitHub/platform brokers remain active through that handoff; the restored product control plane loads only the campaign-free `runtime.env` and may claim general work without a campaign socket. The real activation/stop lifecycle is verified network-free by `verify-product-service-lifecycle.py` and `product_service_lifecycle_test.py`, including systemd masks, service conflicts, shared-port ownership, projector cleanup, transient restart recovery, and persistent restore failure; product operation does not require a gym timer. A new campaign window still requires a new operator-granted campaign identifier and a new dark-install/review cycle.
...
Release `2026.803.0-gloops.35` Paperclip image `sha256:09bf3feac584c2d3a7a33b6d3df6cd3b425a5b27c4b72724c35af492d68615b3` was built from merge `b3233636a20790097f6c082b64fc34bbd2065a33` (host exclusive-writer lock, PR #255), passed workflow `30779371921` attempt 1, and has GitHub build provenance bound to that digest; mutable tag `sha-b323363` is a discovery aid only. This release clears the source-only release-pin interlock after binding the published immutable image and carries the host-wide exclusive writer controller (`paperclip-hostctl.py`) into the install envelope without widening campaign, provider, or admission authority. Product remains paused unless separately authorized.

Release `2026.802.0-gloops.34` Paperclip image `sha256:fb63ebfd959890457a5b80728a35a71da267cb18ff4e4f524768c62c9d1ba53d` was built from merge `63ef21ad2bc962559a2bcb79ecb52f177149aa84`, passed workflow `30768936124` attempt 1, and has GitHub build provenance bound to that digest; mutable tag `sha-63ef21a` is a discovery aid only. Its Hermes execution image remains `sha256:0049638c8554593bfc4dfa767408e04ff2c0caa1b6b94208034a538842f45106`, retained in the root-only archive whose SHA-256 is `3e6f7c3ee31e087ef6b135dac756474eb967070b8722da0f36c894f4e42889f3`. This release freezes the named backlog-bankrupt company by default, keeps readmission explicitly empty, and adds an auditable fail-closed admission denial without widening campaign or provider authority. It retains the guarded reset path, exact requested-model binding, fail-closed route evidence, requested and resolved model truth in terminal receipts, broker-mode credential lifecycle recovery, one-run/one-push authority boundary, atomic settlement, authoritative Hermes route-and-usage receipts, and certified recovery and fail-dark boundaries. It is a bounded reliability release and does not itself authorize recurring autonomy, authority widening, or provider fallback outside the Ollama → Grok CLI → Codex policy.

Hermes execution image `sha256:0049638c8554593bfc4dfa767408e04ff2c0caa1b6b94208034a538842f45106` preserves the live charter-pack, capacity-allocation, and terminal-reconciliation work while adding compatibility between the existing authoritative route-receipt boundary and Hermes' native `moa` virtual provider. It admits a native MoA preset only when the acting model and every reference model resolve to the subscription-included Ollama Cloud provider, records the real acting route, and reuses Hermes' own combined advisor-plus-actor usage snapshot for the terminal receipt rather than maintaining a second accounting path. The root-only archive is `hermes-execution-0049638c8554593bfc4dfa767408e04ff2c0caa1b6b94208034a538842f45106.tar.zst` with SHA-256 `3e6f7c3ee31e087ef6b135dac756474eb967070b8722da0f36c894f4e42889f3`.
...
an isolated harmless transient systemd target. It proves the logical epoch is
root-owned, immutable, non-renewing across restart, stops the target at
expiry, and denies later admission without starting Paperclip or invoking a
provider. The epoch length it certifies defaults to the installed broker's
default and can be set with `--logical-duration-seconds`; the clock is simulated,
so a 30-day epoch takes no longer to rehearse than a 1-hour one. Activation
accepts only a recent receipt bound to the exact installed broker, stop actuator,
and approved image, and whose certified epoch length equals the duration the
installed deadman unit is configured to enforce. It consumes the one-use operator
...
### Ordinary execution baseline

Ordinary product work follows Paperclip's upstream assignment, workspace, run,
and atomic issue-claim path. GLoops does not add a company freeze or per-issue
readmit list in front of that path. Resource budgets remain issue policy, not a
host configuration gate. Controlled-swarm commissioning remains available only
for an explicitly governed campaign; it is not the default admission mechanism
for normal work.
