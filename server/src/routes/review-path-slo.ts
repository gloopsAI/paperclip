...
import {
  ...
  reviewPathSloService,
} from "../services/review-path-slo.js";
...
export function reviewPathSloRoutes(db: Db) {
  const router = Router();
  const svc = reviewPathSloService(db);

  router.get("/companies/:companyId/review-path-slo", async (req, res) => {
    ...
    try {
      resolveReviewPathSloWindow({ since: sinceRaw, until: untilRaw });
    } catch (err) {
      throw badRequest(err instanceof Error ? err.message : "invalid date window");
    }

    const report = await svc.forCompany(companyId, {
      since: sinceRaw,
      until: untilRaw,
    });
    res.json(report);
