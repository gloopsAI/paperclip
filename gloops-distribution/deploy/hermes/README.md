...
- The isolated handshake does not publish a Docker host port: Docker omits host publication when a container is attached only to an `--internal` bridge. Its readiness barrier instead proves the bridge is still internal, the container still owns fixed address `172.30.241.4`, and the host can reach `/api/health` directly over that bridge while preserving the allowed loopback Host value. This keeps host control-plane access without adding an Internet-capable network.
