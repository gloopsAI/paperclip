...
def mint_board_approval(repository_id, repo, base_ref, base_sha, pr, head, substrate_hits, review_issue_id, review_run_id):
    ...
    payload={
        ...
        "recommendedAction":f"Approve to clear GitHub check gloops/independent-review for PR #{pr} and allow GitHub's single-entry merge queue. Reject keeps merge blocked.",
        ...
        "risks":["Touches trust-substrate denylist paths.","Clearance unblocks the independent-review check and single-entry merge queue.","Does not authorize GO DEPLOY or Induct write."],
