...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not project comment-optional successful runs to a missing-disposition blocker", async () => {
    const { companyId, agentId, runId, issueId } = await seedQueuedIssueRunFixture();
    await db
      .update(issues)
      .set({
    ...
    mockAdapterExecute.mockImplementationOnce(async () => ({
      ...
      timedOut: false,
      errorMessage: null,
      summary: "GLO-1132 replay: provider completed successfully with comments disabled.",
      provider: "test",
      model: "test-model",
    }));
    const heartbeat = heartbeatService(db);
    ...
    expect(settledRun?.status).toBe("succeeded");
    expect(settledRun?.contextSnapshot).toMatchObject({ skipIssueComment: true });
  ...
  it("queues one admissible handoff wake under a strict envelope with room for a second run", async () => {
    ...
    await withExecutionEnvironment(generousEnv, async () => {
      ...
      expect(handoffWakeups).toHaveLength(1);
      expect(handoffWakeups[0]?.idempotencyKey).toBe(idempotencyKey);
