...
Release `2026.717.0-gloops.15` image `sha256:7314eedaebf3f167ba3cb2f007e8c7ab66c42c62b7a0ebfb97b0801f913c37b1` was built from merge `bdd506f1dae20ac4c52bc7134a1788d3ae5279fc`, passed workflow `29607159716`, and has GitHub build-provenance attestation. It retains the certified execution, repository, credential, WIP, historical-isolation, recovery, non-renewing campaign-deadman, inert activation, plugin orchestration, wake idempotency, observation, stop, and one-use commissioning boundaries while adding event-driven plugin job retirement: empty job manifests are synchronized before replacement-worker startup, removed schedules are paused and unregistered, and the closed-interval zero-work proof rejects any plugin job run. The release remains a bounded dark-install candidate; it does not itself authorize Paperclip work, agent unpause, provider execution, recurring autonomy, or MTE. Installed deadman and zero-provider startup rehearsals must pass before inert activation, and commissioning still requires a fresh exact-image root-owned approval.
