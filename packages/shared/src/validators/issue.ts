...
const createAcceptedPlanDecompositionChildSchema = withCreateIssueStatusDefault(
  createChildIssueBaseSchema.extend({
    // Accepted-plan siblings may reference one another through blockedByIssueIds,
    // so this surface alone accepts a caller-preallocated durable UUID.
    id: z.string().uuid().optional(),
  }),
);
