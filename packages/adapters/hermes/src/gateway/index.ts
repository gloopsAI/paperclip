...
- subscriptionClass (string): operator-pinned subscription provenance, such as ollama-max.
- routingReason (string): operator-pinned reason this route was selected.
- fallbackOccurred (boolean): operator-pinned fallback observation. Omit when unknown.
- executionProfile (string): operator-pinned execution profile, such as execution-only.
...
- Persists runner=hermes_gateway plus configured subscriptionClass, routingReason,
  fallbackOccurred, and executionProfile into execution_route. These provenance
  fields are never inferred from the model or path and remain absent when not configured.
