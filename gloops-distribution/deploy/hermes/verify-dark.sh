...
readonly IMAGE='ghcr.io/gloopsai/paperclip-gloops@sha256:fb63ebfd959890457a5b80728a35a71da267cb18ff4e4f524768c62c9d1ba53d'
failed=0
...
if [[ -e /etc/paperclip-gloops/CONTROLLED_SWARM_COMMISSIONING_APPROVED ]] \
  ...
  || [[ -e /var/lib/paperclip-gloops/controlled-swarm/commissioning-rollback.json ]] \
...
if grep -Fxq 'PAPERCLIP_RUNTIME_RELEASE_PIN_REQUIRED=false' /etc/paperclip-gloops/runtime.env \
  ...
  echo "PASS controlled-swarm envelope is installed dark with the Gloops company frozen and no successor readmit"
else
