...
- Provider transport certification uses a separate `paperclip-hermes-handshake` unit. Its exact Hermes profile resolves the API-server surface to zero tools, enforces one turn, has no repository, workspace, session, or GitHub mount, persists no runtime state, publishes no host port, and stops after at most 15 minutes. The unit is independently masked while dark and is not a general execution worker.
