...
Release `2026.719.0-gloops.20` image `sha256:ad6096051abe123bc275a11d7904bc53d6c8cb812411b49c142066c00aaab972` was built from merge `23f2f9ebfe31d394593f102fe1b6dffc752ece54`, passed workflow `29670439295`, and has GitHub build-provenance attestation `36003801`; mutable tag `sha-23f2f9e` is a discovery aid only. It retains the certified execution, repository, credential, WIP, historical-isolation, recovery, inert activation, exact-topology rehearsal, successor campaign identity, and fail-dark boundaries while clearing only the bounded Paperclip unit's systemd restart-rate state immediately before recovery restarts. Manual stop also tolerates unloaded-unit reset cleanup only when the complete dark verifier succeeds. This release contains the narrow recovery correction; it does not claim that the host rehearsals have run or that Gate 2 is accepted. The release remains a bounded dark-install candidate and does not itself authorize Paperclip work, agent unpause, provider execution, recurring autonomy, or MTE.
