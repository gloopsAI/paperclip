...
describeEmbeddedPostgres("productivity review service", () => {
  ...
  it("suppresses creation after three completed reviews with no source action", async () => {
    const now = new Date("2026-04-28T12:00:00.000Z");
    const seeded = await seedAssignedIssue();
    await insertRuns({
      companyId: seeded.companyId,
  ...
  it("resets no-action suppression when the source issue has activity", async () => {
    const now = new Date("2026-04-28T12:00:00.000Z");
    const seeded = await seedAssignedIssue();
    await insertRuns({
      companyId: seeded.companyId,
    ...
    await db.insert(activityLog).values({
      ...
      createdAt: new Date(reviewWindows[1]!.createdAt.getTime() + 60_000),
    });
