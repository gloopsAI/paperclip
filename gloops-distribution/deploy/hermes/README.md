...
Release `2026.719.0-gloops.22` image `sha256:8a3a1220ef978e066f1f380eba7565ee6884c34ded06c73a4b090effe3bef895` was built from merge `3a15c109b3d6e05d0aeb0396a97fa21c91159021`, passed workflow `29674699918`, and has GitHub build-provenance attestation `36017412`; mutable tag `sha-3a15c10` is a discovery aid only. It retains the certified execution, repository, credential, WIP, historical-isolation, recovery, inert activation, exact-topology rehearsal, successor campaign identity, and fail-dark boundaries while admitting a retained commissioning rollback journal only during the exact root-locked `barrier_enabled` restart transition with matching approval evidence. Orphaned, malformed, wrong-phase, and approval-mismatched journals remain fail-closed. This release contains the narrow commissioning-startup correction; it does not claim that the host rehearsals have run or that Gate 2 is accepted. The release remains a bounded dark-install candidate and does not itself authorize Paperclip work, agent unpause, provider execution, recurring autonomy, or MTE.
