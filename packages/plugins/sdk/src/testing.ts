        ...
        async getOrchestration(input) {
          ...
          return {
            ...
            runtimeIdentity: {
              ...
              deploymentReceiptDigest: "0".repeat(64),
              jobDeclarationCount: manifest.jobs?.length ?? 0,
              jobKeys: (manifest.jobs ?? []).map((job) => job.jobKey).sort(),
              jobDeclarationsSha256: "0".repeat(64),
            },
            subtreeIssueIds,
  ...
  const harness: TestHarness = {
    ...
    async executeTool<T = ToolResult>(name: string, params: unknown, runCtx: Partial<ToolRunContext> = {}) {
      ...
      const ctxToPass: ToolRunContext = {
        ...
        ...(runCtx.issueId ? { issueId: runCtx.issueId } : {}),
      };
