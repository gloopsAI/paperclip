...
- GitHub publication uses the dedicated `zach-hermes` service identity through read-only credential mounts. Pilot readiness verifies that identity has write access only to the declared public `gloopsAI/gloops-paperclip-plugin` repository, while the pilot charter further restricts each mutation; it does not mount Zach's administrative credential.
