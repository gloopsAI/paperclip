...
- The control-plane scheduler is enabled only to drive bounded scheduled retries and deterministic recovery. Agent timer heartbeats remain disabled in company state, legacy routines/triggers remain paused, and the issue-created cutoff prevents historical scheduler work from becoming eligible.
...
Release `2026.716.0-gloops.10` image `sha256:a3b605c0eeeb73ea1b4813e755f59571234e8640a215cb01523dd2ef7267909d` was built from merge `5208ac5cb18fec960c6536351d47870b1ad54e0a`, passed workflow `29543906993`, and has GitHub build-provenance attestation. It retains the certified execution and private GitHub App credential boundaries while adding reproducible disposable reference-path evidence, exact runtime-source attribution, Hermes startup-update egress removal, and an executable systemd runtime-deadman proof. The release remains a bounded dark-install candidate; it does not authorize Paperclip work, Hermes scheduling, provider execution, recurring autonomy, or MTE. A zero-provider handshake rehearsal must prove host reachability, coordinator/worker inertness, zero created work, zero provider traffic, graceful shutdown, and restoration to the fully verified dark state before one separately chartered provider handshake may be considered.
