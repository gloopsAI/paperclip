...
Release `2026.718.0-gloops.18` image `sha256:5cfcc6ad73406d764a78f9259293281dde4bad3bb3949469e99739a78f761a64` was built from merge `b4cb86d99f3edaf179470890521e6f5428817647`, passed workflow `29660629309` attempt 2, and has GitHub build-provenance attestation `35988977`; mutable tag `sha-b4cb86d` is a discovery aid only. It retains the certified execution, repository, credential, WIP, historical-isolation, recovery, non-renewing campaign-deadman, inert activation, plugin orchestration, wake idempotency, observation, stop, one-use commissioning, event-driven plugin-job-retirement, bounded Hermes accounting, compact-charter commissioning, and fail-dark interrupted-commissioning recovery boundaries while adding an exact-installed-topology SIGKILL recovery rehearsal. The proof requires the real installed wrapper and commissioner to crash and recover through the installed root-owned recovery unit at every durable phase, binds loaded systemd properties and credential lifecycle history, and emits an exact content-addressed receipt only after the conjunctive matrix passes. This release contains the proof mechanism; it does not claim that a host rehearsal has run or that Gate 2 is accepted. The release remains a bounded dark-install candidate and does not itself authorize Paperclip work, agent unpause, provider execution, recurring autonomy, or MTE.
